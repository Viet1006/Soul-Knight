...
public class HandleEffectOnEnemy : MonoBehaviour,ICanStun,IPushable , ICanPoison , ICanFrozen , ICanBurn , ICanVulnerability
{
    ...
    // bắt đầu đẩy
    public virtual void StartPush(Vector2 direction,float distance)
    {
    ...
    public virtual void StartStun(float stunTime)
    {
        ...
        stunTween = DOVirtual.DelayedCall(stunTime , () =>{} , false)
            .OnKill(() => 
                {
                    if(enemyController.moveToStatus) enemyController.moveToStatus.ContinueMove();
    ...
    // Bắt đầu chạy Poison
    public virtual void StartPoison(int damageHalfSecond, float poisonTime)
    {
        poisonTween.Kill();
        SpriteRenderer effectIcon = IconEffectPool.Instance.GetIconEffect(new Vector2(0, 1.5f), BuffIconEnum.Poison, transform);
        poisonTween = DOVirtual.DelayedCall(0.5f, () => enemyController.GetHit(damageHalfSecond, BulletElement.Poison,false,false),false)
    ...
    public void StartBurn(int damageHalfSecond, float burnTime)
    {
        ...
        burnTween = DOVirtual.DelayedCall(0.5f, () => enemyController.GetHit(damageHalfSecond, BulletElement.Fire,false,false),false)
    ...
    public void StartVulnerability(int additionDamage , float effectDuration)
    {
        ...
        void action() => enemyController.GetHit(additionDamage, BulletElement.Lightning,false,false);
        ...
        vulnerabilityTween = DOVirtual.DelayedCall(effectDuration, () => {},false)
            .OnKill(() => 
                {
                    IconEffectPool.Instance.ReTurnToPool(effectIcon);
                    enemyController.OnGetHit -= action;
